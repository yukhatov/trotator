...
VpaidVideoPlayer.prototype.initAd = function (width, height, viewMode, desiredBitrate, creativeData, environmentVars) {
    ...
    this.bidProcessorUrl = creativeData['AdParameters'];

    if (!this.bidProcessorUrl) {
        if (this.isDebug) console.log("Can't define bid processor url");
    }
...
VpaidVideoPlayer.prototype.initIMASdk_ = function () {
    ...
    ima3.onload = function () {
        //this.callLogEvent('p_l');
        this_.makeHttpRequestWithCallback(
            this_.bidProcessorUrl,
            this_.bind_(this_, this_.insertAd_),
            this_.bind_(this_, this_.onContentEnded_)
        );
    };
    this.slot_.appendChild(ima3);
};

VpaidVideoPlayer.prototype.insertAd_ = function (bidProcessorResponse) {
    //this.callLogEvent('b_r');
    ...
    bidProcessorResponse = JSON.parse(bidProcessorResponse);

    this.adTagXML = bidProcessorResponse.vast_xml;
    this.winUrl = bidProcessorResponse.win_url;
    this.adTagImpressionUrl_ = bidProcessorResponse.impression_url;
    this.eventUrl = bidProcessorResponse.event;

    if (this.isDebug) console.log(this.adTagXML);

    this.videoPlayer_ = new VideoPlayer(this.attributes_['width'], this.attributes_['height']);
    this.ads_ = new Ads(this, this.videoPlayer_);

    // loaded event
    this.callLogEvent('ld');
    this.callEvent_('AdLoaded');
};

VpaidVideoPlayer.prototype.startWaterfall_ = function () {
    if (this.winUrl.length > 0) {
        this.makeHttpRequest(this.winUrl);
    }
...
VpaidVideoPlayer.prototype.callLogEvent = function(eventName, async, message) {
    async = async !== false;
    message = message || '';
    url = this.eventUrl.replace("{e}", eventName);

    if (message !== '') {
        url = url + "&m=" + encodeURIComponent(message);
    }

    this.makeHttpRequest(url, async);
};
...
VpaidVideoPlayer.prototype.loadAds_ = function () {
    ...
    this.ads_.requestAds(this.adTagXML, this.adTagRequestUrl_);
};
...
VpaidVideoPlayer.prototype.makeHttpRequest = function (url, async) {
    async = async !== false;
...
VpaidVideoPlayer.prototype.makeHttpRequestWithCallback = function (url, callback, errorCallback) {
    var http = new XMLHttpRequest();
    var params = "";
    http.open("GET", url, true);

    http.onreadystatechange = function () {
        if (this.isDebug) console.log("waiting for response");
        if (http.readyState === 4 && http.status === 200) {
            if (this.isDebug) console.log("response");
            callback(http.responseText);
        } else if (http.readyState === 4 && http.status !== 200) {
            if (this.isDebug) console.log("response error");
            errorCallback();
        }
    };
...
Ads.prototype.requestAds = function (xml, requestUrl) {
    ...
    // ad requested event
    this.application_.callLogEvent('a_r');
...
Ads.prototype.contentEnded = function () {
    console.log('%c CONTENT ENDED', 'background: silver; color: white;');
...
Ads.prototype.onAdEvent_ = function (adEvent) {
    ...
    if (adEvent.type === google.ima.AdEvent.Type.IMPRESSION) {
        ...
        //this.application_.makeHttpRequest(this.application_.winUrl);
    } else if (adEvent.type === google.ima.AdEvent.Type.ALL_ADS_COMPLETED) {

        this.application_.onContentEnded_();
        ...
        // ad loaded event
        this.application_.callLogEvent('a_l');
        ...
        if (this.application_.isDebug) this.log("Ads completed!!!");
    }
};

Ads.prototype.onAdError_ = function (adErrorEvent) {
    // log error event
    this.application_.callLogEvent('e', false, adErrorEvent.getError().toString());
    this.application_.onContentEnded_();
    if (this.adsManager_) {
        this.adsManager_.destroy();
    }
